...
class Segment:
    def __init__(self, segment_id, my_id, priority, cost, peer_ports, neighbor_timeout):
        ...
        self.peers = {
            port: {"peer_id": None, "priority": 0, "state": DOWN, "last_hello": 0}
    ...
    def _eligible(self):
        eligible = {}
        if self.priority > 0:
            eligible[self.my_id] = self.priority
        for entry in self.peers.values():
            if entry["state"] != DOWN and entry["peer_id"] and entry["priority"] > 0:
                eligible[entry["peer_id"]] = entry["priority"]
        return eligible
    ...
    def process_hello(self, peer_port, peer_id, peer_priority):
        with self.lock:
